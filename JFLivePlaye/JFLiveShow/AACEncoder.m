...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer timeStamp:(uint64_t)timeStamp completionBlock:(void (^)(NSData *encodedData, NSError* error))completionBlock {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        JFAudioFrame *audioFrame = [JFAudioFrame new];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(jf_AACEncoder_call_back_audioFrame:)]) {
            [self.delegate jf_AACEncoder_call_back_audioFrame:audioFrame];
        }
